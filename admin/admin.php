...
$total_result = $dbconnect->query("SELECT COUNT(*) AS total
    ...
    WHERE ur.role_id=3 $keyword_sql");
$total_row = $total_result->fetch_assoc();
$total_records = $total_row['total'];
$total_pages = max(1, ceil($total_records / $limit));

// Lấy dữ liệu admin
$sql_admin = "SELECT us.*, ua.username, ua.password FROM user us
    INNER JOIN user_role ur ON us.user_id=ur.user_id
    INNER JOIN user_account ua ON us.user_id=ua.user_id
    WHERE ur.role_id=3 $keyword_sql
    ORDER BY us.user_id DESC
    LIMIT $limit OFFSET $offset";
$result_admin = $dbconnect->query($sql_admin);
?>
<!DOCTYPE html>
<html lang="vi">
<head>
...
<div class="main p-4" id="mainContent">
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
        <h3 class="mb-2">Danh sách Admin</h3>
        <a class="btn btn-primary mb-2" href="account_add.php?role_id=3&role_name=admin">
            <i class="fa-solid fa-plus"></i> Tạo mới
        </a>
    </div>

    <!-- Tìm kiếm + Số dòng hiển thị -->
    <form class="row g-2 align-items-center mb-4" action="admin.php" method="GET">
        <div class="col-auto">
                ...
                <tbody>
                    ...
                    while($row = $result_admin->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-info btn-sm mb-1" href="account_view.php?user_id=<?= $row['user_id'] ?>&role_id=3&role_name=admin"><i class="fa-solid fa-eye"></i> Xem</a>
                                <a class="btn btn-warning btn-sm mb-1" href="account_edit.php?user_id=<?= $row['user_id'] ?>&role_id=3&role_name=admin"><i class="fa-solid fa-pen"></i> Sửa</a>
