<?php
// student_content.php
// Trang xem nội dung khóa học cho người học
...
<head>
    ...
    <title>Nội dung khóa học - Học viên</title>
...
<header class="container mt-4">
    <div class="row">
        ...
        <div class="col-md-6">
            <form class="d-flex" action="student_content.php" method="POST">
                <div class="input-group mb-3">
                    <input type="search" class="form-control" placeholder="Tìm kiếm học liệu ..." name="tukhoa" value="<?php echo htmlspecialchars($search_display); ?>">
                    <button class="btn btn-secondary" type="submit" name="timkiem">Tìm kiếm</button>
                </div>
            </form>
            <?php if ($search_mode) : ?>
                                    ...
                                    <div class="card h-100 shadow-sm position-relative">
                                        <div class="card-body d-flex flex-column">
                                            ...
                                            <div class="mt-auto">
                                                ...
                                                // Trỏ đến folder content trong student
                                                $base_path = "";
                                                switch ($row['content_type']) {
                                                    case 'embed': 
                                                        $url = !empty($row['embedded_id']) ? $base_path . 'view_content_video.php?content_id=' . urlencode($row['embedded_id']) : ''; 
                                                        $button_text = "Xem video";
                                                        break;
                                                    case 'video': 
                                                        $url = !empty($row['video_id']) ? $base_path . 'view_content_file_vd.php?content_id=' . urlencode($row['video_id']) : ''; 
                                                        $button_text = "Xem video";
                                                        break;
                                                    case 'text': 
                                                        $url = !empty($row['text_id']) ? $base_path . 'view_content_text.php?content_id=' . urlencode($row['text_id']) : ''; 
                                                        $button_text = "Đọc tài liệu";
                                                        break;
                                                    case 'file': 
                                                        $url = !empty($row['file_id']) ? $base_path . 'view_content_file.php?content_id=' . urlencode($row['file_id']) : ''; 
