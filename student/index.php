            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    if (isset($_SESSION['user_id'])) {
                        ...
                        $check_joined_sql = "SELECT * FROM course_member 
                                            WHERE student_id = $user_id 
                                            AND course_id = $course_id";
                        $joined_result = mysqli_query($dbconnect, $check_joined_sql);
                        $has_joined = mysqli_num_rows($joined_result) > 0;
                    }
            ?>
                                ...
                                <div class="mt-auto">
                                    ...
                                    <div class="btn-group-custom">
                                        ...
                                        <?php if (isset($_SESSION['user_id'])) { ?>
                                            <?php if ($has_joined): ?>
                                                <!-- Nếu đã tham gia, hiển thị nút Đã tham gia -->
                                                <a class="btn btn-joined" href="course/index.php?id=<?php echo $row['course_id']; ?>">
                                                    Đã tham gia
                                                </a>
                                            <?php else: ?>
                                                <?php if ($is_free): ?>
                                                    <!-- Nếu chưa tham gia và là khóa học miễn phí -->
                                                    <a class="btn btn-success" href="join_free_course.php?course_id=<?php echo $row['course_id']; ?>">
                                                        Tham gia ngay
                                                    </a>
                                                <?php else: ?>
